...
<header class="app-header">
	...
	: $panel[$user]["DATABASES"] ?>&#13;<?= _("Suspended") ?>: <?= $panel[$user]["SUSPENDED_DB"] ?>">
 	...
 	"SUSPENDED_DB"
				...
				<!-- MongoDB tab (VHestiaCP) -->
				<?php if (isset($_SESSION["MONGODB_SYSTEM"]) && !empty($_SESSION["MONGODB_SYSTEM"])) { ?>
					<?php 
					// Get actual MongoDB database count
					$mongo_dbs = 0;
					$mongo_output = [];
					exec(HESTIA_CMD . "v-list-database-mongo " . escapeshellarg($user) . " json 2>/dev/null", $mongo_output, $mongo_return);
					if ($mongo_return === 0 && !empty($mongo_output)) {
						$mongo_data = json_decode(implode("", $mongo_output), true);
						if (is_array($mongo_data)) {
							$mongo_dbs = count($mongo_data);
						}
					}
					?>
					<li class="main-menu-item">
						<a class="main-menu-item-link <?php if ($TAB == "MONGODB") { echo "active"; } ?>" href="/list/mongodb/" title="<?= _("MongoDB Databases") ?>">
							<p class="main-menu-item-label"><?= _("MONGO") ?><i class="fas fa-leaf"></i></p>
							<ul class="main-menu-stats">
								<li>
									<?= _("Databases") ?>: <?= $mongo_dbs ?>
								</li>
							</ul>
						</a>
					</li>
