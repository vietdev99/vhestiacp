...
// Use v-start-pm2-app script which handles PM2_HOME correctly
$cmd = HESTIA_CMD . "v-start-pm2-app " . escapeshellarg($target_user) . " " . escapeshellarg($id) . " 2>&1";
exec($cmd, $output, $return_var);

if ($return_var === 0) {
	$_SESSION["ok_msg"] = sprintf(_("PM2 process %s has been started."), htmlspecialchars($id));
} else {
	$_SESSION["error_msg"] = sprintf(_("Failed to start PM2 process: %s"), implode("\n", $output));
}
